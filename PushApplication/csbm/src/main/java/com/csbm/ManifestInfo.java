...
/** package */ class ManifestInfo {
  private static final String TAG = "com.csbm.ManifestInfo";

  // BEPushBroadcastReceiver intents: ACTION_PUSH_RECEIVE, ACTION_PUSH_OPEN, ACTION_PUSH_DELETE
  ...
  private static boolean usesPushBroadcastReceivers() {
    ...
    if (hasIntentReceiver(BEPushBroadcastReceiver.ACTION_PUSH_RECEIVE)) {
      intentsRegistered++;
    }

    if (hasIntentReceiver(BEPushBroadcastReceiver.ACTION_PUSH_OPEN)) {
      intentsRegistered++;
    }

    if (hasIntentReceiver(BEPushBroadcastReceiver.ACTION_PUSH_DELETE)) {
      intentsRegistered++;
    }

    if (intentsRegistered != 0 && intentsRegistered != NUMBER_OF_PUSH_INTENTS) {
      throw new SecurityException(
          "The BE Push BroadcastReceiver must implement a filter for all of " +
                  BEPushBroadcastReceiver.ACTION_PUSH_RECEIVE + ", " +
                  BEPushBroadcastReceiver.ACTION_PUSH_OPEN + ", and " +
      ...
      if (pushType == null) {
        ...
        if (hasPushBroadcastReceiver
          ...
          if (hasAnyGcmSpecificDeclaration) {
             ...
             * BEPushBroadcastReceiver. */
              PLog.e(TAG, "Push is currently disabled. This is probably because you migrated " +
                  "from an older version of BE. This version of BE requires your app to " +
                  "have a BroadcastReceiver that handles " +
                      BEPushBroadcastReceiver.ACTION_PUSH_RECEIVE + ", " +
                      BEPushBroadcastReceiver.ACTION_PUSH_OPEN + ", and " +
                      BEPushBroadcastReceiver.ACTION_PUSH_DELETE + ". You can do this by adding " +
                  "these lines to your AndroidManifest.xml:\n\n" +
                  " <receiver android:name=\"com.csbm.BEPushBroadcastReceiver\"\n" +
         ...
         */
        if (CSBM.getLogLevel() <= CSBM.LOG_LEVEL_WARNING) {
          if (pushType == PushType.GCM && gcmSupportLevel == ManifestCheckResult.MISSING_OPTIONAL_DECLARATIONS) {
            PLog.w(TAG, "Using GCM for BE Push, but the app manifest is missing some optional " +
  ...
  private static Context getContext() {
    return CSBM.getApplicationContext();
  }
  ...
  private static ManifestCheckResult ppnsSupportLevel() {
    ...
    if (!checkReceiver(BEBroadcastReceiver.class, null, intents)) {
      return ManifestCheckResult.MISSING_OPTIONAL_DECLARATIONS;
    }
  ...
  private static String getPpnsManifestMessage() {
    return "make sure that these permissions are declared as children of the root " +
        ...
        "\n" +
        "<service android:name=\"com.csbm.PushService\" />\n" +
        "<receiver android:name=\"com.csbm.BEBroadcastReceiver\">\n" +
        "  <intent-filter>\n" +
