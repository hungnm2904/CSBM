...
/** package */ class PushRouter {
  private static final String TAG = "com.csbm.BEPushRouter";
  ...
  public static synchronized PushRouter getInstance() {
    if (instance == null) {
      File diskState = new File(BEPlugins.get().getFilesDir(), STATE_LOCATION);
      File oldDiskState = new File(BEPlugins.get().getBEDir(), LEGACY_STATE_LOCATION);
  ...
  /* package for tests */ static synchronized void resetInstance() {
    BEFileUtils.deleteQuietly(new File(BEPlugins.get().getFilesDir(), STATE_LOCATION));
    instance = null;
  }
  ...
  /* package for tests */ static PushRouter pushRouterFromState(
    ...
    if (didMigrate) {
      ...
      BEFileUtils.deleteQuietly(oldDiskState);
    }
  ...
  private synchronized void saveStateToDisk() {
    try {
      BEFileUtils.writeJSONObjectToFile(diskState, toJSON());
    } catch (IOException | JSONException e) {
  ...
  public synchronized boolean handlePush(
    ...
    if (BETextUtils.isEmpty(pushId) || BETextUtils.isEmpty(timestamp)) {
      return false;
    }
    ...
    extras.putString(BEPushBroadcastReceiver.KEY_PUSH_CHANNEL, channel);
    if (data == null) {
      extras.putString(BEPushBroadcastReceiver.KEY_PUSH_DATA, "{}");
    } else {
      extras.putString(BEPushBroadcastReceiver.KEY_PUSH_DATA, data.toString());
    }

    Intent intent = new Intent(BEPushBroadcastReceiver.ACTION_PUSH_RECEIVE);
