...
@implementation BEQueryUtilities
///--------------------------------------
...
+ (NSPredicate *)_mapPredicate:(NSPredicate *)predicate
  ...
  BEConsistencyAssertionFailure(@"NSExpression predicates are not supported.");
  return nil;
}
  ...
  return [self _mapPredicate:predicate
               compoundBlock:^NSPredicate *(NSCompoundPredicate *compound) {
                 switch (compound.compoundPredicateType) {
                   ...
                   default: {
                     BEConsistencyAssertionFailure(@"This compound predicate cannot be negated. (%zd)",
                 ...
                 switch (comparison.predicateOperatorType) {
                     ...
                     case NSBetweenPredicateOperatorType: {
                       BEConsistencyAssertionFailure(@"A BETWEEN predicate was found after they should have been removed.");
                     }
                   ...
                   default: {
                     BEConsistencyAssertionFailure(@"This comparison predicate cannot be negated. (%@)", comparison);
                     return nil;
                   }
                 }
  ...
  return [self _mapPredicate:predicate
             ...
             comparisonBlock:^NSPredicate *(NSComparisonPredicate *predicate) {
               if (predicate.predicateOperatorType == NSBetweenPredicateOperatorType) {
                 ...
                 BEConsistencyAssert(rhs.expressionType == NSConstantValueExpressionType ||
                                     rhs.expressionType == NSAggregateExpressionType,
                                     @"The right-hand side of a BETWEEN operation must be a value or literal.");
                 
                 BEConsistencyAssert([rhs.constantValue isKindOfClass:[NSArray class]],
                                     @"The right-hand side of a BETWEEN operation must be an array.");
                 
                 NSArray *array = rhs.constantValue;
                 BEConsistencyAssert(array.count == 2, @"The right-hand side of a BETWEEN operation must have 2 items.");
...
+ (NSPredicate *)asOrOfAnds:(NSCompoundPredicate *)compound {
    ...
    for (NSPredicate *subpredicate in dnfSubpredicates) {
      ...
      if ([subpredicate isKindOfClass:[NSCompoundPredicate class]]) {
        ...
        if (subcompound.compoundPredicateType == NSOrPredicateType) {
          ...
          BEConsistencyAssertionFailure(@"[BEQuery asOrOfAnds:] found a compound query that wasn't OR or AND.");
        }
      } else {
  ...
  BEConsistencyAssertionFailure(@"[BEQuery asOrOfAnds:] was passed a compound query that wasn't OR or AND.");
  return nil;
}
...
+ (void)assertNoPredicateModifiers:(NSPredicate *)predicate {
  [self _mapPredicate:predicate
        compoundBlock:nil
      comparisonBlock:^NSPredicate *(NSComparisonPredicate *comparison) {
        BEConsistencyAssert(comparison.comparisonPredicateModifier == NSDirectPredicateModifier,
...
/**
 Returns a version of the given predicate converted to disjunctive normal form (DNF),
 known colloqially as an "or of ands", the only form of query that BEQuery accepts.
 */
...
+ (NSError *)objectNotFoundError {
  return [BEErrorUtilities errorWithCode:kBEErrorObjectNotFound message:@"No results matched the query."];
}
@end
