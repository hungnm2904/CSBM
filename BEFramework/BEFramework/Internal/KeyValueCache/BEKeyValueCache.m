...
#import "BEKeyValueCache_Private.h"
...
static const NSUInteger BEKeyValueCacheDefaultDiskCacheSize = 10 << 20;
static const NSUInteger BEKeyValueCacheDefaultDiskCacheRecords = 1000;
static const NSUInteger BEKeyValueCacheDefaultMemoryCacheRecordSize = 1 << 20;
static const NSTimeInterval BEKeyValueCacheDiskCacheTimeResolution = 1; // HFS+ stores only second level accuracy.

static NSString *const BEKeyValueCacheDiskCachePathKey = @"path";

@interface BEKeyValueCacheEntry ()
...
@implementation BEKeyValueCacheEntry
...
@implementation BEKeyValueCache {
...
- (instancetype)initWithCacheDirectoryURL:(NSURL *)url
  ...
  _maxDiskCacheBytes = BEKeyValueCacheDefaultDiskCacheSize;
  _maxDiskCacheRecords = BEKeyValueCacheDefaultDiskCacheRecords;
  _maxMemoryCacheBytesPerRecord = BEKeyValueCacheDefaultMemoryCacheRecordSize;
...
- (void)setObject:(NSString *)value forKey:(NSString *)key {
  ...
  if ((keyBytes + valueBytes) < self.maxMemoryCacheBytesPerRecord) {
    [self.memoryCache setObject:[BEKeyValueCacheEntry cacheEntryWithValue:value] forKey:key];
  } else {
...
- (NSString *)objectForKey:(NSString *)key maxAge:(NSTimeInterval)maxAge {
  ...
  BEKeyValueCacheEntry *cacheEntry = [self.memoryCache objectForKey:key];
  ...
  dispatch_sync(_diskCacheQueue, ^{
    ...
    [self.memoryCache setObject:[BEKeyValueCacheEntry cacheEntryWithValue:value creationTime:modificationDate]
                         forKey:key];
  });
...
- (BOOL)_isDiskCacheDirty {
  ...
  return (actualInterval - knownInterval) >= BEKeyValueCacheDiskCacheTimeResolution;
}
...
- (void)_addToDiskCacheDictionary:(NSString *)path modificationDate:(NSDate *)modificationDate size:(NSUInteger)size {
  NSDictionary *entry = @{
                          BEKeyValueCacheDiskCachePathKey: path,
...
- (void)_compactDiskCache {
  ...
  while (_lastDiskCacheAttributes.count > _maxDiskCacheRecords || _lastDiskCacheSize > _maxDiskCacheBytes) {
    ...
    NSString *toRemove = attributes[BEKeyValueCacheDiskCachePathKey];
