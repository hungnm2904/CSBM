...
static NSString *csbmServer_;

@implementation BEInternalUtils

+ (void)initialize {
  if (self == [BEInternalUtils class]) {
    [self setCSBMServer:_CSBMDefaultServerURLString];
  }
}
...
+ (void)appendObject:(id)object toString:(NSMutableString *)string {
  if ([object isKindOfClass:[NSDictionary class]]) {
    ...
    BEParameterAssertionFailure(@"Couldn't create cache key from %@", object);
  }
}
...
+ (id)traverseObject:(id)object usingBlock:(id (^)(id object))block seenObjects:(NSMutableSet *)seen {
  if ([object isKindOfClass:[BEObject class]]) {
    ...
    // chua code cho nay ben beobject
//    for (NSString *key in ((BEObject *)object).allKeys) {
...
+ (id)_stringWithFormat:(NSString *)format arguments:(NSArray *)arguments {
  ...
  BERangeAssert(arguments.count <= maxNumArgs, @"Maximum of %d format args allowed", maxNumArgs);
