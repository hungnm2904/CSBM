...
/**
 * Manages a set of local ids and possible mappings to global CSBM objectIds. This class is
 * thread-safe.
 */
...
/** package */ class LocalIdManager {
  ...
  private synchronized MapEntry getMapEntry(String localId) {
    ...
    try {
      JSONObject json = BEFileUtils.readFileToJSONObject(new File(diskPath, localId));
  ...
  synchronized String createLocalId() {
    ...
    if (!isLocalId(localId)) {
      throw new IllegalStateException("Generated an invalid local id: \"" + localId + "\". "
          + "This should never happen. Contact us at https://csbm.com/help");
    }
