...
/** package */ class BESQLiteDatabase {
  ...
  /* protected */ static Task<BESQLiteDatabase> openDatabaseAsync(final SQLiteOpenHelper helper, int flags) {
    final BESQLiteDatabase db = new BESQLiteDatabase(flags);
    return db.open(helper).continueWithTask(new Continuation<Void, Task<BESQLiteDatabase>>() {
      @Override
      public Task<BESQLiteDatabase> then(Task<Void> task) throws Exception {
   ...
   */
  private BESQLiteDatabase(int flags) {
    ...
    synchronized (currentLock) {
      Task<Cursor> task = current.onSuccess(new Continuation<Void, Cursor>() {
        ...
        public Cursor then(Task<Cursor> task) throws Exception {
          Cursor cursor = BESQLiteCursor.create(task.getResult(), dbExecutor);
