...
/**
 * BECommandCache manages an on-disk cache of commands to be executed, and a thread with a
...
/** package */ class BECommandCache extends BEEventuallyQueue {
  private static final String TAG = "com.csbm.BECommandCache";
  ...
  // that if somehow there are two instances of BECommandCache, they won't step on each others'
  ...
  private static File getCacheDir() {
    ...
    File cacheDir = new File(CSBM.getBEDir(), "CommandCache");
    cacheDir.mkdirs();
  ...
  ConnectivityNotifier.ConnectivityListener listener = new ConnectivityNotifier.ConnectivityListener() {
    ...
    public void networkConnectivityStatusChanged(Context context, Intent intent) {
      ...
      /*
       ...
       TODO(grantland): Convert to TaskQueue, similar to BEPinningEventuallyQueue
        */
  ...
  public BECommandCache(Context context, BEHttpClient client) {
    setConnected(false);
    ...
    if (!CSBM.hasPermission(Manifest.permission.ACCESS_NETWORK_STATE)) {
  ...
  public void resume() {
    synchronized (runningLock) {
      if (!running) {
        new Thread("BECommandCache.runLoop()") {
          @Override
          public void run() {
            runLoop();
          }
  ...
  private void removeFile(File file) {
    synchronized (lock) {
      ...
      try {
        ...
        BERESTCommand command = commandFromJSON(json);
  ...
  @Override
  public Task<JSONObject> enqueueEventuallyAsync(BERESTCommand command,
                                                 BEObject object) {
   ...
   *          - See runEventually.
   */
  private Task<JSONObject> enqueueEventuallyAsync(BERESTCommand command, boolean preferOldest,
                                                  BEObject object) {
    ...
    synchronized (lock) {
      try {
        ...
        BEFileUtils.writeByteArrayToFile(path, json);
  ...
  /**
   * This is kind of like BETaskUtils.wait(), except that it gives up the CommandCache's lock
   * while the task is running, and reclaims it before returning.
   */
  private <T> T waitForTaskWithoutLock(Task<T> task) throws BEException {
    synchronized (lock) {
      ...
      return BETaskUtils.wait(task);  // Just to get the return value and maybe rethrow.
    }
  }
   ...
   * but csbm can't be reached, waits timeoutRetryWaitSeconds before retrying up to
    ...
    synchronized (lock) {
      ...
      for (String fileName : fileNames) {
        ...
        try {
          ...
          if (CSBM.LOG_LEVEL_ERROR >= CSBM.getLogLevel()) {
            log.log(Level.SEVERE, "Unable to create BECommand from JSON.", e);
          }
          removeFile(file);
          continue;
        }
        ...
        try {
          ...
          if (e.getCode() == BEException.CONNECTION_FAILED) {
            if (retriesRemaining > 0) {
              // Reachability says we have a network connection, but we can't actually contact
              // CSBM. Wait N minutes, or until we get signaled again before doing anything else.
              if (CSBM.LOG_LEVEL_INFO >= CSBM.getLogLevel()) {
   ...
   * lock.notify(). Usually that happens as a result of either (1) CSBM being initialized, (2)
   * runEventually being called, or (3) the OS notifying that the network connection has been
   * re-established.
   */
  private void runLoop() {
    if (CSBM.LOG_LEVEL_INFO >= CSBM.getLogLevel()) {
      log.info("CSBM command cache has started processing queued commands.");
    }
