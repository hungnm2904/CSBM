...
/**
 * Encodes {@link BEObjects} as pointers. If the object does not have an objectId, throws an
 * exception.
 */
...
/** package */ class PointerEncoder extends PointerOrLocalIdEncoder {
  ...
  public JSONObject encodeRelatedObject(BEObject object) {
    // Ensure the BEObject has an id so it can be encoded as a pointer.
    if (object.getObjectId() == null) {
      // object that hasn't been saved.
      throw new IllegalStateException("unable to encode an association with an unsaved BEObject");
    }
