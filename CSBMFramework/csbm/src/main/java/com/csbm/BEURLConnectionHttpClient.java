...
/** package */ class BEURLConnectionHttpClient extends BEHttpClient<HttpURLConnection, HttpURLConnection> {
  ...
  public BEURLConnectionHttpClient(int socketOperationTimeout, SSLSessionCache sslSessionCache) {
  ...
  /* package */ BEHttpResponse executeInternal(BEHttpRequest beRequest) throws IOException {
    HttpURLConnection connection = getRequest(beRequest);
  ...
  @Override
  /* package */ HttpURLConnection getRequest(BEHttpRequest beRequest)
      throws IOException {
    HttpURLConnection connection;
    URL url = new URL(beRequest.getUrl());

    connection = (HttpURLConnection)url.openConnection();

    connection.setRequestMethod(beRequest.getMethod().toString());
  ...
  @Override
  /* package */ BEHttpResponse getResponse(HttpURLConnection connection)
      throws IOException {
    // Status code
    ...
    for (Map.Entry<String, List<String>> entry : connection.getHeaderFields().entrySet()) {
      ...
      // have already had statusCode in BEHttpResponse, we just ignore this header entry.
