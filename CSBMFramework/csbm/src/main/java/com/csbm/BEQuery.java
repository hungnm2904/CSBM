...
/**
 * The {@code BEQuery} class defines a query that is used to fetch {@link BEObject}s. The most
 ...
 * </pre>
 * <p/>
 * A {@code BEQuery} can also be used to retrieve a single object whose id is known, through the
 ...
 * </pre>
 * <p/>
 * A {@code BEQuery} can also be used to count the number of objects that match the query without
 ...
 */
public class BEQuery<T extends BEObject> {

  private static BEQueryController getQueryController() {
    return BECorePlugins.getInstance().getQueryController();
  }
  ...
  /**
   * Constraints for a {@code BEQuery}'s where clause. A map of field names to constraints. The
  ...
  /* package */ static class RelationConstraint {
    private String key;
    private BEObject object;

    public RelationConstraint(String key, BEObject object) {
    ...
    public BERelation<BEObject> getRelation() {
     ...
     */
    public JSONObject encode(BEEncoder objectEncoder) {
   ...
   *
   * @param queries
   *          The list of {@code BEQuery}s to 'or' together
   * @return A {@code BEQuery} that is the 'or' of the passed in queries
   */
  public static <T extends BEObject> BEQuery<T> or(List<BEQuery<T>> queries) {
    if (queries.isEmpty()) {
      throw new IllegalArgumentException("Can't take an or of an empty list of queries");
    }

    List<State.Builder<T>> builders = new ArrayList<>();
    for (BEQuery<T> query : queries) {
      builders.add(query.getBuilder());
    }
    return new BEQuery<>(State.Builder.or(builders));
  }

  /**
   * Creates a new query for the given {@link BEObject} subclass type. A default query with no
   * further parameters will retrieve all {@link BEObject}s of the provided class.
   *
   * @param subclass
   *          The {@link BEObject} subclass type to retrieve.
   * @return A new {@code BEQuery}.
   */
  public static <T extends BEObject> BEQuery<T> getQuery(Class<T> subclass) {
    return new BEQuery<>(subclass);
  }

  /**
   * Creates a new query for the given class name. A default query with no further parameters will
   * retrieve all {@link BEObject}s of the provided class name.
   *
   * @param className
   *          The name of the class to retrieve {@link BEObject}s for.
   * @return A new {@code BEBQuery}.
   */
  public static <T extends BEObject> BEQuery<T> getQuery(String className) {
    return new BEQuery<>(className);
  }

  /**
   * Constructs a query for {@link BEUser}s.
   *
   * @deprecated Please use {@link BEUser#getQuery()} instead.
   */
  @Deprecated
  public static BEQuery<BEUser> getUserQuery() {
    return BEUser.getQuery();
  }
   ...
   * network requests have not yet had time to complete. BE takes care of automatically flushing
  ...
  public enum CachePolicy {
     ...
     * <p/>
     * If there are no cached results, this causes a {@link BEException#CACHE_MISS}.
     */
    CACHE_ONLY,
  ...
  private static void throwIfLDSEnabled(boolean enabled) {
    boolean ldsEnabled = CSBM.isLocalDatastoreEnabled();
  ...
  /* package */ static class State<T extends BEObject> {

    /* package */ static class Builder<T extends BEObject> {

      // TODO(grantland): Convert mutable parameter to immutable t6941155
      public static <T extends BEObject> Builder<T> or(List<Builder<T>> builders) {
        if (builders.isEmpty()) {
          throw new IllegalArgumentException("Can't take an or of an empty list of queries");
        }
       ...
       *          The key to check.
       * @param value
       *          The value that the {@link BEObject} must contain.
      ...
      public Builder<T> whereNear(String key, BEGeoPoint point) {
      ...
      public Builder<T> whereWithin(String key, BEGeoPoint southwest, BEGeoPoint northeast) {
      ...
      // Used by BERelation
      /* package */ Builder<T> whereRelatedTo(BEObject parent, String key) {
        where.put("$relatedTo", new RelationConstraint(key, parent));
        return this;
      }

      /**
       * Add a constraint that a require matches any one of an array of {@code BEQuery}s.
       * <p/>
       * The {@code BEQuery}s passed cannot have any orders, skips, or limits set.
       *
       * @param constraints
      ...
      /**
       * Include nested {@link BEObject}s for the provided key.
       * <p/>
      ...
      /**
       * Restrict the fields of returned {@link BEObject}s to only include the provided keys.
       * <p/>
      ...
      public Builder<T> fromPin() {
        return fromPin(BEObject.DEFAULT_PIN);
      }
    ...
    /* package */ JSONObject toJSON(BEEncoder encoder) {
      ...
      try {
        ...
        if (!order.isEmpty()) {
          params.put("order", BETextUtils.join(",", order));
        }
        if (!include.isEmpty()) {
          params.put("include", BETextUtils.join(",", include));
        }
        if (selectedKeys != null) {
          params.put("fields", BETextUtils.join(",", selectedKeys));
        }
        if (trace) {
          params.put("trace", 1);
        }
  ...
  /**
   * Constructs a query for a {@link BEObject} subclass type. A default query with no further
   * parameters will retrieve all {@link BEObject}s of the provided class.
   *
   * @param subclass
   *          The {@link BEObject} subclass type to retrieve.
   */
  public BEQuery(Class<T> subclass) {
    this(getSubclassingController().getClassName(subclass));
  }

  /**
   * Constructs a query. A default query with no further parameters will retrieve all
   * {@link BEObject}s of the provided class.
   *
   * @param theClassName
   *          The name of the class to retrieve {@link BEObject}s for.
   */
  public BEQuery(String theClassName) {
   ...
   *          The query to copy.
   */
  public BEQuery(BEQuery<T> query) {
    this(new State.Builder<>(query.getBuilder()));
    user = query.user;
  }

  /* package */ BEQuery(State.Builder<T> builder) {
   ...
   */
  /* package for tests */ BEQuery<T> setUser(BEUser user) {
    this.user = user;
    return this;
  }
   ...
   * enabled.
   */
  /* package for tests */ Task<BEUser> getUserAsync(State<T> state) {
    if (state.ignoreACLs()) {
    ...
    return BEUser.getCurrentUserAsync();
  }
  ...
  /**
   * Retrieves a list of {@link BEObject}s that satisfy this query.
   * <p/>
   * @return A list of all {@link BEObject}s obeying the conditions set in this query.
   * @throws BEException
   *           Throws a {@link BEException} if no object is found.
   *
   * @see BEException#OBJECT_NOT_FOUND
   */
  public List<T> find() throws BEException {
    return BETaskUtils.wait(findInBackground());
  }

  /**
   * Retrieves at most one {@link BEObject} that satisfies this query.
   * <p/>
   * <strong>Note:</strong>This mutates the {@code BEQuery}.
   *
   * @return A {@link BEObject} obeying the conditions set in this query.
   * @throws BEException
   *           Throws a {@link BEException} if no object is found.
   *
   * @see BEException#OBJECT_NOT_FOUND
   */
  public T getFirst() throws BEException {
    return BETaskUtils.wait(getFirstInBackground());
  }
   ...
   * @return this, so you can chain this call.
   *
   * @see BEQuery#fromLocalDatastore()
   * @see BEQuery#fromPin()
   * @see BEQuery#fromPin(String)
   */
  public BEQuery<T> setCachePolicy(CachePolicy newCachePolicy) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   *
   * @see BEQuery#setCachePolicy(CachePolicy)
   */
  /* package */ BEQuery<T> fromNetwork() {
    checkIfRunning();
   ...
   * @see BEQuery#setCachePolicy(CachePolicy)
   */
  public BEQuery<T> fromLocalDatastore() {
   ...
   * @return this, so you can chain this call.
   *
   * @see BEObject#DEFAULT_PIN
   * @see BEQuery#setCachePolicy(CachePolicy)
   */
  public BEQuery<T> fromPin() {
    checkIfRunning();
    builder.fromPin();
    return this;
  }
   ...
   * @see BEQuery#setCachePolicy(CachePolicy)
   */
  public BEQuery<T> fromPin(String name) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> setMaxCacheAge(long maxAgeInMilliseconds) {
    checkIfRunning();
  ...
  /**
   * Retrieves a list of {@link BEObject}s that satisfy this query from the source in a
   * background thread.
   * <p/>
  ...
  public void findInBackground(final FindCallback<T> callback) {
    ...
    if (state.cachePolicy() != CachePolicy.CACHE_THEN_NETWORK ||
      ...
      task = doCacheThenNetwork(state, callback, new CacheThenNetworkCallable<T, Task<List<T>>>() {
        @Override
        public Task<List<T>> call(State<T> state, BEUser user, Task<Void> cancellationToken) {
          return findAsync(state, user, cancellationToken);
        }
      });
    }
    BETaskUtils.callbackOnMainThreadAsync(task, callback);
  }
  ...
  private Task<List<T>> findAsync(final State<T> state) {
    return doWithRunningCheck(new Callable<Task<List<T>>>() {
      ...
      public Task<List<T>> call() throws Exception {
        return getUserAsync(state).onSuccessTask(new Continuation<BEUser, Task<List<T>>>() {
          @Override
  ...
  /* package */ Task<List<T>> findAsync(State<T> state, BEUser user, Task<Void> cancellationToken) {
    return BEQuery.getQueryController().findAsync(state, user, cancellationToken);
  }

  /**
   * Retrieves at most one {@link BEObject} that satisfies this query from the source in a
   * background thread.
   * <p/>
  ...
  public void getFirstInBackground(final GetCallback<T> callback) {
    ...
    if (state.cachePolicy() != CachePolicy.CACHE_THEN_NETWORK ||
      ...
      task = doCacheThenNetwork(state, callback, new CacheThenNetworkCallable<T, Task<T>>() {
        @Override
        public Task<T> call(State<T> state, BEUser user, Task<Void> cancellationToken) {
  ...
  private Task<T> getFirstAsync(final State<T> state) {
    return doWithRunningCheck(new Callable<Task<T>>() {
      ...
      public Task<T> call() throws Exception {
        return getUserAsync(state).onSuccessTask(new Continuation<BEUser, Task<T>>() {
          @Override
          public Task<T> then(Task<BEUser> task) throws Exception {
            final BEUser user = task.getResult();
  ...
  private Task<T> getFirstAsync(State<T> state, BEUser user, Task<Void> cancellationToken) {
    return BEQuery.getQueryController().getFirstAsync(state, user, cancellationToken);
  }
   ...
   */
  public int count() throws BEException {
    return BETaskUtils.wait(countInBackground());
  }
  ...
  public void countInBackground(final CountCallback callback) {
    ...
    final BECallback2<Integer, BEException> c = callback != null
        ? new BECallback2<Integer, BEException>() {
            @Override
            public void done(Integer integer, BEException e) {
    ...
    if (state.cachePolicy() != CachePolicy.CACHE_THEN_NETWORK ||
      ...
      task = doCacheThenNetwork(state, c, new CacheThenNetworkCallable<T, Task<Integer>>() {
        @Override
        public Task<Integer> call(State<T> state, BEUser user, Task<Void> cancellationToken) {
          return countAsync(state, user, cancellationToken);
        }
      });
    }
    BETaskUtils.callbackOnMainThreadAsync(task, c);
  }
  ...
  private Task<Integer> countAsync(final State<T> state) {
    return doWithRunningCheck(new Callable<Task<Integer>>() {
      ...
      public Task<Integer> call() throws Exception {
        return getUserAsync(state).onSuccessTask(new Continuation<BEUser, Task<Integer>>() {
          @Override
          public Task<Integer> then(Task<BEUser> task) throws Exception {
            final BEUser user = task.getResult();
  ...
  private Task<Integer> countAsync(State<T> state, BEUser user, Task<Void> cancellationToken) {
    return BEQuery.getQueryController().countAsync(state, user, cancellationToken);
  }

  /**
   * Constructs a {@link BEObject} whose id is already known by fetching data from the source.
   * <p/>
   * <strong>Note:</strong>This mutates the {@code BEQuery}.
   *
   * @param objectId
   *          Object id of the {@link BEObject} to fetch.
   * @throws BEException
   ...
   * @see BEException#OBJECT_NOT_FOUND
   */
  public T get(final String objectId) throws BEException {
    return BETaskUtils.wait(getInBackground(objectId));
  }
  ...
  public boolean hasCachedResult() {
    ...
    BEUser user = null;
    try {
      user = BETaskUtils.wait(getUserAsync(state));
    } catch (BEException e) {
      // do nothing
    }
     ...
     * cache.
     */
    String raw = BEKeyValueCache.loadFromKeyValueCache(
        BERESTQueryCommand.findCommand(state, sessionToken).getCacheKey(), state.maxCacheAge()
    );
    return raw != null;
  }
  ...
  public void clearCachedResult() {
    ...
    BEKeyValueCache.clearFromKeyValueCache(
        BERESTQueryCommand.findCommand(state, sessionToken).getCacheKey()
    );
  }
  ...
  public static void clearAllCachedResults() {
    ...
    BEKeyValueCache.clearKeyValueCacheDir();
  }

  /**
   * Constructs a {@link BEObject} whose id is already known by fetching data from the source in a
   * background thread. This does not use caching.
   * <p/>
   * This is preferable to using the {@link BEObject#createWithoutData(String, String)}, unless
  ...
  /**
   * Constructs a {@link BEObject} whose id is already known by fetching data from the source in
  ...
  private <TResult> Task<TResult> doCacheThenNetwork(
      ...
      final BECallback2<TResult, BEException> callback,
    ...
    return doWithRunningCheck(new Callable<Task<TResult>>() {
      ...
      public Task<TResult> call() throws Exception {
        return getUserAsync(state).onSuccessTask(new Continuation<BEUser, Task<TResult>>() {
          @Override
          public Task<TResult> then(Task<BEUser> task) throws Exception {
            final BEUser user = task.getResult();
            ...
            executionTask = BETaskUtils.callbackOnMainThreadAsync(executionTask, callback);
  ...
  private interface CacheThenNetworkCallable<T extends BEObject, TResult> {
    TResult call(BEQuery.State<T> state, BEUser user, Task<Void> cancellationToken);
  }
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereEqualTo(String key, Object value) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereLessThan(String key, Object value) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereNotEqualTo(String key, Object value) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereGreaterThan(String key, Object value) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereLessThanOrEqualTo(String key, Object value) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereGreaterThanOrEqualTo(String key, Object value) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereContainedIn(String key, Collection<? extends Object> values) {
    checkIfRunning();
   ...
   * {@code BEQuery}.
   * <p/>
   * This only works on keys whose values are {@link BEObject}s or lists of {@link BEObject}s.
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereContainsAll(String key, Collection<?> values) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereMatchesQuery(String key, BEQuery<?> query) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereDoesNotMatchQuery(String key, BEQuery<?> query) {
    checkIfRunning();
   ...
   * in the results of another {@code BEQuery}.
   *
   * @param key
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereMatchesKeyInQuery(String key, String keyInQuery, BEQuery<?> query) {
    checkIfRunning();
   ...
   * for a key in the results of another {@code BEQuery}.
   *
   * @param key
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereDoesNotMatchKeyInQuery(String key, String keyInQuery,
                                                   BEQuery<?> query) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereNotContainedIn(String key, Collection<? extends Object> values) {
    checkIfRunning();
   ...
   *
   * @param key
   *          The key that the {@link BEGeoPoint} is stored in.
   * @param point
   *          The reference {@link BEGeoPoint} that is used.
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereNear(String key, BEGeoPoint point) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereWithinMiles(String key, BEGeoPoint point, double maxDistance) {
    checkIfRunning();
    return whereWithinRadians(key, point, maxDistance / BEGeoPoint.EARTH_MEAN_RADIUS_MILE);
  }
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereWithinKilometers(String key, BEGeoPoint point, double maxDistance) {
    checkIfRunning();
    return whereWithinRadians(key, point, maxDistance / BEGeoPoint.EARTH_MEAN_RADIUS_KM);
  }
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereWithinRadians(String key, BEGeoPoint point, double maxDistance) {
    checkIfRunning();
    builder.whereNear(key, point)
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereWithinGeoBox(
          String key, BEGeoPoint southwest, BEGeoPoint northeast) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereMatches(String key, String regex) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereMatches(String key, String regex, String modifiers) {
    checkIfRunning();
    builder.addCondition(key, "$regex", regex);
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereContains(String key, String substring) {
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereStartsWith(String key, String prefix) {
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereEndsWith(String key, String suffix) {
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> include(String key) {
    checkIfRunning();
    builder.include(key);
    return this;
  }
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> selectKeys(Collection<String> keys) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereExists(String key) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> whereDoesNotExist(String key) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> orderByAscending(String key) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> addAscendingOrder(String key) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> orderByDescending(String key) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> addDescendingOrder(String key) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> setLimit(int newLimit) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> setSkip(int newSkip) {
    checkIfRunning();
   ...
   * @return this, so you can chain this call.
   */
  public BEQuery<T> setTrace(boolean shouldTrace) {
    checkIfRunning();
