...
/**
 * A BEException gets raised whenever a {@link BEObject} issues an invalid request, such as
 * deleting or editing an object that no longer exists on the server, or when there is a network
 * failure preventing communication with the CSBM server.
 */
public class BEException extends Exception {
  ...
  /**
   * Error code indicating the connection to the CSBM servers failed.
   */
   ...
   * about changing internal CSBM code.
   */
  ...
  /**
   * You must call CSBM.initialize before using the CSBM library.
   */
  ...
  /**
   * Error code indicating that an invalid filename was used for BEFile. A valid file name
  ...
  /**
   * Construct a new BEException with a particular error code.
   *
   * @param theCode
   ...
   */
  public BEException(int theCode, String theMessage) {
    super(theMessage);
    code = theCode;
  }

  /**
   * Construct a new BEException with an external cause.
   *
   * @param message
   *          A message describing the error in more detail.
   * @param cause
   ...
   */
  public BEException(int theCode, String message, Throwable cause) {
    super(message, cause);
    code = theCode;
  }
