...
/**
 * {@code BEFile} is a local representation of a file that is saved to the CSBM cloud.
 * <p/>
 * The workflow is to construct a {@code BEFile} with data and optionally a filename. Then save
 * it and set it as a field on a {@link BEObject}.
 * <p/>
 * Example:
 * <pre>
 * BEFile file = new BEFile("hello".getBytes());
 * file.save();
 *
...
public class BEFile {
  ...
  /* package for tests */ static BEFileController getFileController() {
    return BECorePlugins.getInstance().getFileController();
  }
  ...
  /**
   * Staging of {@code BEFile}'s data is stored in memory until the {@code BEFile} has been
   ...
   */
  public BEFile(File file, String contentType) {
   ...
   *          The file's content type.
   */
  public BEFile(String name, byte[] data, String contentType) {
   ...
   *          The file's data.
   */
  public BEFile(byte[] data) {
    this(null, data, null);
  }

  /**
   * Creates a new file from a byte array and a name. Giving a name with a proper file extension
   * (e.g. ".png") is ideal because it allows CSBM to deduce the content type of the file and set
   ...
   *          The file's data.
   */
  public BEFile(String name, byte[] data) {
    this(name, data, null);
  }
   ...
   *          The file's content type.
   */
  public BEFile(byte[] data, String contentType) {
    this(null, data, contentType);
  }

  /* package for tests */ BEFile(State state) {
    this.state = state;
  }
  ...
  /**
   * Saves the file to the CSBM cloud synchronously.
   */
  public void save() throws BEException {
  ...
  /**
   * Saves the file to the CSBM cloud in a background thread.
  ...
  public void saveInBackground(final SaveCallback saveCallback,
    ...
    BETaskUtils.callbackOnMainThreadAsync(saveInBackground(progressCallback), saveCallback);
  }
   ...
   * background thread.
   */
  public byte[] getData() throws BEException {
    return BETaskUtils.wait(getDataInBackground());
  }
      ...
      public Task<byte[]> then(Task<Void> toAwait) throws Exception {
        return fetchInBackground(progressCallback, toAwait, cts.getTask()).onSuccess(new Continuation<File, byte[]>() {
          ...
          public byte[] then(Task<File> task) throws Exception {
            ...
            try {
              return BEFileUtils.readFileToByteArray(file);
            } catch (IOException e) {
              // do nothing
            }
            return null;
  ...
  public void getDataInBackground(GetDataCallback dataCallback,
      final ProgressCallback progressCallback) {
    BETaskUtils.callbackOnMainThreadAsync(getDataInBackground(progressCallback), dataCallback);
  }
   ...
   */
  public void getDataInBackground(GetDataCallback dataCallback) {
    BETaskUtils.callbackOnMainThreadAsync(getDataInBackground(), dataCallback);
  }
   ...
   */
  public File getFile() throws BEException {
    return BETaskUtils.wait(getFileInBackground());
  }
  ...
  public void getFileInBackground(GetFileCallback fileCallback,
      final ProgressCallback progressCallback) {
    BETaskUtils.callbackOnMainThreadAsync(getFileInBackground(progressCallback), fileCallback);
  }
   ...
   */
  public void getFileInBackground(GetFileCallback fileCallback) {
    BETaskUtils.callbackOnMainThreadAsync(getFileInBackground(), fileCallback);
  }
   ...
   * background thread.
   */
  public InputStream getDataStream() throws BEException {
    return BETaskUtils.wait(getDataStreamInBackground());
  }
   ...
   */
  public void getDataStreamInBackground(GetDataStreamCallback dataStreamCallback) {
    BETaskUtils.callbackOnMainThreadAsync(getDataStreamInBackground(), dataStreamCallback);
  }
  ...
  /**
   * Cancels the operations for this {@code BEFile} if they are still in the task queue. However,
  ...
  /* package */ BEFile(JSONObject json, BEDecoder decoder) {
  ...
  /* package */ JSONObject encode() throws JSONException {
    ...
    String url = getUrl();
    if (url == null) {
      throw new IllegalStateException("Unable to encode an unsaved BEFile.");
    }
    json.put("url", getUrl());
