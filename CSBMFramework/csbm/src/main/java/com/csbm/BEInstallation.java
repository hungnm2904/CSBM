...
/**
 * The {@code BEInstallation} is a local representation of installation data that can be saved
 * and retrieved from the CSBM cloud.
 */
@BEClassName("_Installation")
public class BEInstallation extends BEObject {
  private static final String TAG = "com.csbm.BEInstallation";

  private static final String KEY_INSTALLATION_ID = "installationId";
  private static final String KEY_DEVICE_TYPE = "deviceType";
  private static final String KEY_APP_NAME = "appName";
  private static final String KEY_APP_IDENTIFIER = "appIdentifier";
  private static final String KEY_CSBM_VERSION = "beVersion";
  ...
  private static final List<String> READ_ONLY_FIELDS = Collections.unmodifiableList(
      Arrays.asList(KEY_DEVICE_TYPE, KEY_INSTALLATION_ID, KEY_DEVICE_TOKEN, KEY_PUSH_TYPE,
          KEY_TIME_ZONE, KEY_LOCALE, KEY_APP_VERSION, KEY_APP_NAME, KEY_CSBM_VERSION,
          KEY_APP_IDENTIFIER));

  // TODO(mengyan): Inject into BEInstallationInstanceController
  /* package */ static BECurrentInstallationController getCurrentInstallationController() {
    return BECorePlugins.getInstance().getCurrentInstallationController();
  }

  public static BEInstallation getCurrentInstallation() {
    try {
      return BETaskUtils.wait(
  ...
  /**
   * Constructs a query for {@code BEInstallation}.
   * <p/>
   ...
   */
  public static BEQuery<BEInstallation> getQuery() {
    return BEQuery.getQuery(BEInstallation.class);
  }

  public BEInstallation() {
    // do nothing
  }
  ...
  /* package */ void updateBeforeSave() {
    ...
    if (getCurrentInstallationController().isCurrent(BEInstallation.this)) {
      updateTimezone();
      updateVersionInfo();
      updateDeviceInfo();
    ...
    synchronized (mutex) {
      ...
      return result.onSuccessTask(new Continuation<Void, Task<T>>() {
        ...
        public Task<T> then(Task<Void> task) throws Exception {
          return BEInstallation.super.fetchAsync(sessionToken, toAwait);
        }
      });
    }
  }
  ...
  @Override
  /* package */ Task<Void> handleSaveResultAsync(BEObject.State result,
                                                 BEOperationSet operationsBeforeSave) {
    ...
    return task.onSuccessTask(new Continuation<Void, Task<Void>>() {
      ...
      public Task<Void> then(Task<Void> task) throws Exception {
        return getCurrentInstallationController().setAsync(BEInstallation.this);
      }
    });
  }

  @Override
  /* package */ Task<Void> handleFetchResultAsync(final BEObject.State newState) {
  ...
  private void updateVersionInfo() {
    synchronized (mutex) {
      try {
        Context context = CSBM.getApplicationContext();
      ...
      if (!VERSION_NAME.equals(get(KEY_CSBM_VERSION))) {
        performPut(KEY_CSBM_VERSION, VERSION_NAME);
      }
    }
  }
  ...
  // TODO(mengyan): Move to BEInstallationInstanceController
  /* package */ void updateDeviceInfo() {
    updateDeviceInfo(BEPlugins.get().installationId());
  }
