...
/** package */ class BEDecompressInterceptor implements BENetworkInterceptor {
  ...
  public BEHttpResponse intercept(Chain chain) throws IOException {
    ...
    BEHttpResponse response = chain.proceed(request);
    ...
    if (GZIP_ENCODING.equalsIgnoreCase(response.getHeader(CONTENT_ENCODING_HEADER))) {
      ...
      // TODO(mengyan): Add builder constructor based on an existing BEHttpResponse
      response = new BEHttpResponse.Builder(response)
          .setTotalSize(-1)
