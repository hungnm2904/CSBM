 ...
 * (GCM) or CSBM's own push network. CSBM will inspect your application's manifest at runtime and
 * determine which service to use for push. We recommend using GCM for push on devices that have
 * Google Play Store support. CSBM uses its own push network for apps that want to avoid a
 ...
 * make sure that com.csbm.GcmBroadcastReceiver and com.csbm.PushService are declared as children
 * of the <code>&lt;application&gt;</code> element:
 * <p/>
 * <pre>
 * &lt;service android:name="com.csbm.PushService" /&gt;
 * &lt;receiver android:name="com.csbm.GcmBroadcastReceiver"
 ...
 * <p/>
 * To configure the PushService for CSBM's push network, ensure these permission declarations are
 ...
 * </pre>
 * <p/>
 * Also, make sure that {@link PushService} and {@link BEPushBroadcastReceiver} are declared as
 ...
 * </pre>
 * <p/>
 * Note that you can configure the push service for both GCM and CSBM's network by adding all the
 * declarations above to your application's manifest. In this case, CSBM will use GCM on devices
 * with Play Store support and fall back to using CSBM's network on devices without Play Store
 * support. If you want to customize the way your app generates Notifications for your pushes, you
 * can register a custom subclass of {@link BEPushBroadcastReceiver}.
 * <p/>
 * Once push notifications are configured in the manifest, you can subscribe to a push channel by
 * calling:
 * <p/>
 * <pre>
 * BEPush.subscribeInBackground(&quot;the_channel_name&quot;);
 * </pre>
 * <p/>
 * When the client receives a push message, a notification will appear in the system tray. When the
 * user taps the notification, it will broadcast the &quot;com.csbm.push.intent.OPEN&quot; intent.
 * The {@link BEPushBroadcastReceiver} listens to this intent to track an app open event and
 * launch the app's launcher activity. To customize this behavior override
 * {@link BEPushBroadcastReceiver#onPushOpen(Context, Intent)}.
 */
public final class PushService extends Service {
  private static final String TAG = "com.csbm.PushService";

  /* package */ static final String ACTION_START_IF_REQUIRED =
      "com.csbm.PushService.startIfRequired";
  ...
  public void onCreate() {
    ...
    if (BEPlugins.Android.get().applicationContext() == null) {
      PLog.e(TAG, "The CSBM push service cannot start because CSBM.initialize "
          + "has not yet been called. If you call CSBM.initialize from "
