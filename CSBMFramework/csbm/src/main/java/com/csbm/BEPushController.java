...
/** package */ class BEPushController {
  ...
  public BEPushController(BEHttpClient restClient) {
    this.restClient = restClient;
  }

  public Task<Void> sendInBackground(BEPush.State state, String sessionToken) {
    return buildRESTSendPushCommand(state, sessionToken).executeAsync(restClient).makeVoid();
  }

  /* package for test */ BERESTCommand buildRESTSendPushCommand(BEPush.State state,
                                                                   String sessionToken) {
    ...
    return BERESTPushCommand.sendPushCommand(state.queryState(), state.channelSet(), deviceType,
