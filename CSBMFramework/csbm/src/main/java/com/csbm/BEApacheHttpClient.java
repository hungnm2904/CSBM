...
/**
 * An implementation of BEHttpClient using Apache httpclient library
 */
@SuppressWarnings("deprecation")
/** package */ class BEApacheHttpClient extends BEHttpClient<HttpUriRequest, HttpResponse> {

  private static final String CONTENT_ENCODING_HEADER = "Content-Encoding";

  private DefaultHttpClient apacheClient;

  public BEApacheHttpClient(int socketOperationTimeout, SSLSessionCache sslSessionCache) {
    ...
    // [2] Originally from BECommand, check proxy
  ...
  /* package */ BEHttpResponse executeInternal(BEHttpRequest beRequest) throws IOException {
    HttpUriRequest apacheRequest = getRequest(beRequest);
  ...
  @Override
  /* package */ BEHttpResponse getResponse(HttpResponse apacheResponse)
      throws IOException {
  ...
  @Override
  /* package */ HttpUriRequest getRequest(BEHttpRequest beRequest)
      throws IOException {
    if (beRequest == null) {
      throw new IllegalArgumentException(
          "BEHttpRequest passed to getApacheRequest should not be null."
      );
    }

    HttpUriRequest apacheRequest;
    BEHttpRequest.Method method = beRequest.getMethod();
    String url = beRequest.getUrl();
    switch (method) {
      case GET:
    ...
    // Set entity
    BEHttpBody body = beRequest.getBody();
    switch (method) {
      case POST:
        ((HttpPost) apacheRequest).setEntity(new BEApacheHttpEntity(body));
        break;
      case PUT:
        ((HttpPut) apacheRequest).setEntity(new BEApacheHttpEntity(body));
        break;
      default:
        break;
    }
  ...
  /**
   * An wrapper of Apache InputStreamEntity. It takes a BEHttpBody
   * and transfer it to a HttpEntity
   */
  private static class BEApacheHttpEntity extends InputStreamEntity {
    private BEHttpBody beBody;

    public BEApacheHttpEntity(BEHttpBody beBody) throws IOException {
      super(beBody.getContent(), beBody.getContentLength());
      super.setContentType(beBody.getContentType());
      this.beBody = beBody;
    }
