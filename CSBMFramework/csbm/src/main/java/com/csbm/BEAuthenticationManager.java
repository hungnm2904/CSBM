...
/** package */ class BEAuthenticationManager {

  private final Object lock = new Object();
  private final Map<String, AuthenticationCallback> callbacks = new HashMap<>();
  private final BECurrentUserController controller;

  public BEAuthenticationManager(BECurrentUserController controller) {
  ...
  public void register(final String authType, AuthenticationCallback callback) {
    ...
    if (BEAnonymousUtils.AUTH_TYPE.equals(authType)) {
      // There's nothing to synchronize
      return;
    }

    // Synchronize the current user with the auth callback.
    controller.getAsync(false).onSuccessTask(new Continuation<BEUser, Task<Void>>() {
      @Override
