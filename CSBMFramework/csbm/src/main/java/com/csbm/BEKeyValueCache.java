...
/**
 * Used for BEQuery caching.
 */

/** package */ class BEKeyValueCache {

  private static final String TAG = "BEKeyValueCache";
  private static final String DIR_NAME = "BEKeyValueCache";
  ...
  /* package for tests */ static void initialize(File path) {
    if (!path.isDirectory() && !path.mkdir()) {
      throw new RuntimeException("Could not create BEKeyValueCache directory");
    }
    directory = path;
  }
  ...
  /* package */ static void saveToKeyValueCache(String key, String value) {
    synchronized (MUTEX_IO) {
      ...
      try {
        BEFileUtils.writeByteArrayToFile(f, value.getBytes("UTF-8"));
