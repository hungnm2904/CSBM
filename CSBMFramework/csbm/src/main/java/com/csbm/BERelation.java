 ...
 * of CSBM.Relation is associated with a particular parent object and key.
 */
public class BERelation<T extends BEObject> {
  private final Object mutex = new Object();

  // The owning object of this BERelation.
  private WeakReference<BEObject> parent;

  // The object Id of the parent.
  private String parentObjectId;

  // The classname of the parent to retrieve the parent BEObject in case the parent is GC'ed.
  ...
  private Set<BEObject> knownObjects = new HashSet<>();

  /* package */ BERelation(BEObject parent, String key) {
  ...
  /* package */ BERelation(String targetClass) {
    this.parent = null;
  ...
  /**
   * BEs a relation from JSON.
   */
  /* package */ BERelation(JSONObject jsonObject, BEDecoder decoder) {
    this.parent = null;
    this.parentObjectId = null;
    this.parentClassName = null;
    this.key = null;
    ...
    if (objectsArray != null) {
      for (int i = 0; i < objectsArray.length(); ++i) {
        knownObjects.add((BEObject)decoder.decode(objectsArray.optJSONObject(i)));
      }
    }
  }

  /* package */ void ensureParentAndKey(BEObject someParent, String someKey) {
    synchronized (mutex) {
      if (parent == null) {
      ...
      if (parent.get() != someParent) {
        throw new IllegalStateException(
            "Internal error. One BERelation retrieved from two different BEObjects.");
      }
      if (!key.equals(someKey)) {
        throw new IllegalStateException(
            "Internal error. One BERelation retrieved from two different keys.");
      }
    }
  }
  ...
  public void add(T object) {
    synchronized (mutex) {
      BERelationOperation<T> operation =
          new BERelationOperation<>(Collections.singleton(object), null);
  ...
  public void remove(T object) {
    synchronized (mutex) {
      BERelationOperation<T> operation =
          new BERelationOperation<>(null, Collections.singleton(object));
      targetClass = operation.getTargetClass();
      getParent().performOperation(key, operation);
   ...
   * 
   * @return A BEQuery that restricts the results to objects in this relations.
   */
  public BEQuery<T> getQuery() {
    synchronized (mutex) {
      BEQuery.State.Builder<T> builder;
      if (targetClass == null) {
        builder = new BEQuery.State.Builder<T>(parentClassName)
            .redirectClassNameForKey(key);
      } else {
        builder = new BEQuery.State.Builder<>(targetClass);
      }
      builder.whereRelatedTo(getParent(), key);
      return new BEQuery<>(builder);
    }
  }

  /* package */ JSONObject encodeToJSON(BEEncoder objectEncoder) throws JSONException {
    synchronized (mutex) {
      JSONObject relation = new JSONObject();
      relation.put("__type", "Relation");
      relation.put("className", targetClass);
      JSONArray knownObjectsArray = new JSONArray();
      for (BEObject knownObject : knownObjects) {
        try {
   ...
   */
  /* package */ void addKnownObject(BEObject object) {
    synchronized (mutex) {
      knownObjects.add(object);
    }
  }
   ...
   */
  /* package */ void removeKnownObject(BEObject object) {
    synchronized (mutex) {
      knownObjects.remove(object);
    }
  }
   ...
   * caching.
   */
  /* package */ boolean hasKnownObject(BEObject object) {
    synchronized (mutex) {
      return knownObjects.contains(object);
    }
  }

  /* package for tests */ BEObject getParent() {
    if(parent == null){
      return null;
    }
    if(parent.get() == null){
      return BEObject.createWithoutData(parentClassName, parentObjectId);
    }
    return parent.get();
  }
  ...
  /* package for tests */ Set<BEObject> getKnownObjects() {
    return knownObjects;
  }
}
