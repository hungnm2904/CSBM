...
import com.csbm.BEQuery.KeyConstraints;
import com.csbm.BEQuery.QueryConstraints;
import com.csbm.BEQuery.RelationConstraint;
...
/** package */ class OfflineQueryLogic {
   ...
   * BEObject matches each part of the query. This is done because some parts of the query (such
   * as $inQuery) are much more efficient if we can do some preprocessing. This makes some parts of
   * the query matching stateful.
   */
  /* package */ abstract class ConstraintMatcher<T extends BEObject> {

    /* package */ final BEUser user;

    public ConstraintMatcher(BEUser user) {
      this.user = user;
    }

    /* package */ abstract Task<Boolean> matchesAsync(T object, BESQLiteDatabase db);
  }
   ...
   */
  private static Object getValue(Object container, String key) throws BEException {
    return getValue(container, key, 0);
  }

  private static Object getValue(Object container, String key, int depth) throws BEException {
    if (key.contains(".")) {
       ...
       * anything like BEObjects and arrays.
       */
      if (!(value == null || value == JSONObject.NULL || value instanceof Map || value instanceof JSONObject)) {
        ...
        throw new BEException(BEException.INVALID_QUERY, String.format("Key %s is invalid.",
            key));
      }
      return getValue(value, parts[1], depth + 1);
    }

    if (container instanceof BEObject) {
      final BEObject object = (BEObject) container;

      // The object needs to have been fetched already if we are going to sort by one of its fields.
      if (!object.isDataAvailable()) {
        throw new BEException(BEException.INVALID_NESTED_KEY, String.format("Bad key: %s",
            key));
      }

      // Handle special keys for BEObjects.
      switch (key) {
        case "objectId":
      ...
      throw new BEException(BEException.INVALID_NESTED_KEY, String.format("Bad key: %s", key));
    }
  }
  ...
  private static boolean matchesEqualConstraint(Object constraint, Object value) {
    ...
    if (constraint instanceof BEGeoPoint && value instanceof BEGeoPoint) {
      BEGeoPoint lhs = (BEGeoPoint) constraint;
      BEGeoPoint rhs = (BEGeoPoint) value;
  ...
  private static boolean matchesRegexConstraint(Object constraint, Object value, String options)
    ...
    if (!options.matches("^[imxs]*$")) {
      throw new BEException(BEException.INVALID_QUERY, String.format(
  ...
  private static boolean matchesNearSphereConstraint(Object constraint, Object value,
    ...
    BEGeoPoint point1 = (BEGeoPoint) constraint;
    BEGeoPoint point2 = (BEGeoPoint) value;
  ...
  private static boolean matchesWithinConstraint(Object constraint, Object value)
    ...
    HashMap<String, ArrayList<BEGeoPoint>> constraintMap =
        (HashMap<String, ArrayList<BEGeoPoint>>) constraint;
    ArrayList<BEGeoPoint> box = constraintMap.get("$box");
    BEGeoPoint southwest = box.get(0);
    BEGeoPoint northeast = box.get(1);
    BEGeoPoint target = (BEGeoPoint) value;

    if (northeast.getLongitude() < southwest.getLongitude()) {
      throw new BEException(BEException.INVALID_QUERY,
  ...
  private static boolean matchesStatelessConstraint(String operator, Object constraint,
                                                    Object value, KeyConstraints allKeyConstraints) throws BEException {
    switch (operator) {
      case "$ne":
  ...
  private abstract class SubQueryMatcher<T extends BEObject> extends ConstraintMatcher<T> {
    private final BEQuery.State<T> subQuery;
    private Task<List<T>> subQueryResults = null;

    public SubQueryMatcher(BEUser user, BEQuery.State<T> subQuery) {
      super(user);
      this.subQuery = subQuery;
    }

    @Override
    public Task<Boolean> matchesAsync(final T object, BESQLiteDatabase db) {
      /*
      ...
      return subQueryResults.onSuccess(new Continuation<List<T>, Boolean>() {
        @Override
        public Boolean then(Task<List<T>> task) throws BEException {
          return matches(object, task.getResult());
        }
      });
    }

    protected abstract boolean matches(T object, List<T> results) throws BEException;
  }

  /**
   * Creates a matcher that handles $inQuery constraints.
   */
  private <T extends BEObject> ConstraintMatcher<T> createInQueryMatcher(BEUser user,
                                                                            Object constraint, final String key) {
    // TODO(grantland): Convert builder to state t6941155
    @SuppressWarnings("unchecked")
    BEQuery.State<T> query = ((BEQuery.State.Builder<T>) constraint).build();
    return new SubQueryMatcher<T>(user, query) {
      @Override
      protected boolean matches(T object, List<T> results) throws BEException {
   ...
   */
  private <T extends BEObject> ConstraintMatcher<T> createNotInQueryMatcher(BEUser user,
                                                                               Object constraint, final String key) {
    final ConstraintMatcher<T> inQueryMatcher = createInQueryMatcher(user, constraint, key);
    return new ConstraintMatcher<T>(user) {
      @Override
      public Task<Boolean> matchesAsync(T object, BESQLiteDatabase db) {
   ...
   */
  private <T extends BEObject> ConstraintMatcher<T> createSelectMatcher(BEUser user,
                                                                           Object constraint, final String key) {
    Map<?, ?> constraintMap = (Map<?, ?>) constraint;
    // TODO(grantland): Convert builder to state t6941155
    @SuppressWarnings("unchecked")
    BEQuery.State<T> query = ((BEQuery.State.Builder<T>) constraintMap.get("query")).build();
   ...
   */
  private <T extends BEObject> ConstraintMatcher<T> createDontSelectMatcher(BEUser user,
                                                                               Object constraint, final String key) {
   ...
   */
  private <T extends BEObject> ConstraintMatcher<T> createMatcher(BEUser user,
   ...
   * Handles $or queries.
   */
  private <T extends BEObject> ConstraintMatcher<T> createOrMatcher(BEUser user,
    ...
    return new ConstraintMatcher<T>(user) {
      @Override
      public Task<Boolean> matchesAsync(final T object, final BESQLiteDatabase db) {
   ...
   */
  /* package */ static <T extends BEObject> boolean hasReadAccess(BEUser user, T object) {
    if (user == object) {
      return true;
    }

    BEACL acl = object.getACL();
    if (acl == null) {
      return true;
    }
   ...
   * Returns true iff the object is visible based on its read ACL and the given user objectId.
   */
  /* package */ static <T extends BEObject> boolean hasWriteAccess(BEUser user, T object) {
    if (user == object) {
      return true;
    }
   ...
   */
  /* package */ <T extends BEObject> ConstraintMatcher<T> createMatcher(
          BEQuery.State<T> state, final BEUser user) {
   ...
   */
  /* package */ static <T extends BEObject> void sort(List<T> results, BEQuery.State<T> state)
      throws BEException {
    ...
    for (String key : state.order()) {
      if (!key.matches("^-?[A-Za-z][A-Za-z0-9_]*$")) {
        if (!"_created_at".equals(key) && !"_updated_at".equals(key)) {
          throw new BEException(BEException.INVALID_KEY_NAME, String.format(
    ...
    BEGeoPoint mutableNearSphereValue = null;
    for (String queryKey : state.constraints().keySet()) {
      ...
      if (queryKeyConstraints instanceof KeyConstraints) {
        ...
        if (keyConstraints.containsKey("$nearSphere")) {
          ...
          mutableNearSphereValue = (BEGeoPoint) keyConstraints.get("$nearSphere");
        }
      }
    }
    final String nearSphereKey = mutableNearSphereKey;
    final BEGeoPoint nearSphereValue = mutableNearSphereValue;
    ...
    Collections.sort(results, new Comparator<T>() {
      ...
      public int compare(T lhs, T rhs) {
        if (nearSphereKey != null) {
          ...
          BEGeoPoint rhsPoint;
          try {
            lhsPoint = (BEGeoPoint) getValue(lhs, nearSphereKey);
            rhsPoint = (BEGeoPoint) getValue(rhs, nearSphereKey);
          } catch (BEException e) {
            throw new RuntimeException(e);
          }
  ...
  private static Task<Void> fetchIncludeAsync(
    ...
    if (path == null) {
      if (JSONObject.NULL.equals(container)) {
        ...
        return Task.forResult(null);
      } else if (container instanceof BEObject) {
        BEObject object = (BEObject) container;
        return store.fetchLocallyAsync(object, db).makeVoid();
      } else {
        return Task.forError(new BEException(
                BEException.INVALID_NESTED_KEY, "include is invalid for non-BEObjects"));
      }
    }
        ...
        if (container instanceof BEObject) {
          ...
          return fetchIncludeAsync(store, container, null, db).onSuccess(new Continuation<Void, Object>() {
            ...
            public Object then(Task<Void> task) throws Exception {
              return ((BEObject) container).get(key);
            }
          });
        } else if (container instanceof Map) {
   ...
   */
  /* package */ static <T extends BEObject> Task<Void> fetchIncludesAsync(
      final OfflineStore store,
      final T object,
