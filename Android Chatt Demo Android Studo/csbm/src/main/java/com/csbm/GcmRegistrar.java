...
/** package */ class GcmRegistrar {
  private static final String TAG = "com.csbm.GcmRegistrar";
  private static final String REGISTRATION_ID_EXTRA = "registration_id";
  private static final String ERROR_EXTRA = "error";

  // Client-side key for csmbplatform@gmail.com. See csbm/config/gcm.yml for server-side key.
  private static final String BE_SENDER_ID = "1076345567071";
  private static final String SENDER_ID_EXTRA = "com.csbm.push.gcm_sender_id";
  ...
  private static class Singleton {
    public static final GcmRegistrar INSTANCE = new GcmRegistrar(CSBM.getApplicationContext());
  }
  ...
  public Task<Void> registerAsync() {
    ...
    synchronized (lock) {
       ...
       */
      final BEInstallation installation = BEInstallation.getCurrentInstallation();
  ...
  private Task<Void> sendRegistrationRequestAsync() {
    synchronized (lock) {
      ...
      //
      //   <meta-data android:name="com.csbm.push.gcm_sender_id"
  ...
  public Task<Void> handleRegistrationIntentAsync(Intent intent) {
    ...
    /*
     * We have to csbm the response here because GCM may send us a new registration_id
    ...
    return Task.call(new Callable<Void>() {
      ...
      public Void call() throws Exception {
        synchronized (localDeviceTokenLastModifiedMutex) {
          ...
          try {
            BEFileUtils.writeStringToFile(getLocalDeviceTokenLastModifiedFile(),
      ...
      public Long call() throws Exception {
        synchronized (localDeviceTokenLastModifiedMutex) {
          if (localDeviceTokenLastModified == 0) {
            try {
              String localDeviceTokenLastModifiedStr = BEFileUtils.readFileToString(
  ...
  /** package for tests */ static File getLocalDeviceTokenLastModifiedFile() {
    File dir = CSBM.getBECacheDir("GCMRegistrar");
  ...
  /** package for tests */ static void deleteLocalDeviceTokenLastModifiedFile() {
    BEFileUtils.deleteQuietly(getLocalDeviceTokenLastModifiedFile());
  }
  ...
  private static class Request {
    private static final String RETRY_ACTION = "com.csbm.RetryGcmRegistration";
